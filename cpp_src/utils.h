...
std::map< EliasFanoDB::CellTypeName, std::map<int, Transaction> >  transposeResultToCell(const py::list& genes_results);


std::set<Pattern> FPGrowthFrequentItemsetMining(const py::dict& genes_results, const unsigned int min_support_cutoff);
std::set<Pattern> exhaustiveFrequentItemsetMining(const py::dict& gene_results, const unsigned int min_support_cutoff);
